...
export class RasterTile extends THREE.EventDispatcher {
    ...
    load(requester, view) {
        if (this.state.canTryUpdate()) {
            ...
            // TODO: handle the case has no data in source ?
            ...
            return view.mainLoop.scheduler.execute(command).then((textures) => {
                ...
                .catch(err => handlingError(err, requester, this.layer, nextLevel, view, this.state));
        }
    ...
    deltaZoomData() {
        const currentLevel = this.level === EMPTY_TEXTURE_ZOOM ? 0 : this.level;
        const targetLevel = this.tiles[0].zoom;
