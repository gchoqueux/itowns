...
class TiledGeometryLayer extends GeometryLayer {
    ...
    constructor(id, object3d, schemeTile, builder, config) {
         ...
         * @type {number}
         */
        this.maxDeltaElevationLevel = maxDeltaElevationLevel || 5;
    ...
    update(context, layer, node) {
        ...
        if (node.visible && this.subdivision(context, node)) {
            ...
            // display if children aren't ready yet
    ...
    subdivideNode(context, node) {
        if (!node.pendingSubdivision && node.children.length === 0) {
            node.pendingSubdivision = true;

            // Defer child creation to break the recursive updateElements chain
            return Promise.resolve().then(() => {
                if (node.parent) {
                    node.extent.subdivision().forEach((extent) => {
    ...
    subdivision(context, node) {
        ...
        if (!node.material.dataHasLoaded() || node.pendingSubdivision) {
            return false;
        }

        const currentElevationTile = node.material.getCurrentElevationTile();
        if (currentElevationTile && currentElevationTile.deltaZoomData() > this.maxDeltaElevationLevel) {
            return false;
        }
