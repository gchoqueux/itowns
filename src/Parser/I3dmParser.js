...
export default {
    ...
    parse(buffer, options) {
        ...
        if (b3dmHeader.magic) {
            ...
            b3dmHeader.gltfFormat = view.getUint32(byteOffset, true);
            if (b3dmHeader.gltfFormat !== 1 && b3dmHeader.gltfFormat !== 0) {
                throw new Error(`Only glTF format 0 (uri) or 1 (embedded) are supported. Format ${b3dmHeader.gltfFormat} is not.`);
            }
            byteOffset += Uint32Array.BYTES_PER_ELEMENT;
            ...
            var gltfByteLength = 0 + b3dmHeader.byteLength - posGltf;
            if (gltfByteLength === 0) {
                throw new Error('glTF byte length is zero, i3dm must have a glTF to instance.');
            }


            const gltfBuffer = buffer.slice(posGltf, gltfByteLength + posGltf);
            const gltfView = new Uint8Array(buffer, posGltf, gltfByteLength);

            if (b3dmHeader.gltfFormat == 0) {
                // it's uri
                const url = utf8Decoder.decode(new Uint8Array(gltfView));
                const gltfUrl = url.replace(/[\s\0]+$/, '');
                promises.push(new Promise((resolve/* , reject */) => {
                    ...
                    glTFLoader.load(gltfUrl, onload);
                }));
            } else {
                ...
                promises.push(new Promise((resolve/* , reject */) => {
                    const onload = (gltf) => {
                        ...
                        const contentArray = new Uint8Array(gltfBuffer, 20);
            ...
            throw new Error('Invalid i3dm file.');
        }
    },
};
