...
import readPBF from 'Parser/PbfParser';
...
addEventListener('message', (e) => {
    ...
    readPBF(params.file, params.options).then((collection) => {
        ...
        if (mesh && mesh.type == 'Group') {
            const mes = mesh.children.map((m) => {
                ...
                m.geometry.boundingSphere.set(2048, 2048, 0);
                m.geometry.boundingSphere.radius = 2900;
                return {
                    type: m.type,
                    color: m.geometry.attributes.color.array,
