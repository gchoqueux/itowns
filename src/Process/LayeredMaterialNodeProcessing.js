...
export function updateLayeredMaterialNodeImagery(context, layer, node, parent) {
    ...
    if (node.layerUpdateState[layer.id] === undefined) {
        ...
        if (!nodeLayer) {
            ...
            nodeLayer.initFromParent(parentLayer);
        }
...
export function updateLayeredMaterialNodeElevation(context, layer, node, parent) {
    ...
    // /!\
    // pour le chargement de niveau inferieur :
    // Une alternative est chargé le noeud parent et ensuite d'affecter
    //
    ...
    return context.scheduler.execute(command).then(
        (textures) => {
            ...
            if (targetLevel < nodeLayer.level || !texture) {
                node.layerUpdateState[layer.id].noMoreUpdatePossible();
                return;
            }
            const elevation = {
